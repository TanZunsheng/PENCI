...
class PhysicsDecoder(nn.Module):
    ...
    额外支持 projection-only 模式：
    - 仅保留显式状态 `S_t -> leadfield 投影 -> 传感器` 接口
    - 不注册 temporal_decoder 参数，适合 V1 当前主路径
    - 该模式下不支持 `forward(source_activity, ...)`
    ...
    def __init__(
        ...
        if self.projection_only and not use_fixed_leadfield:
            raise ValueError("projection_only 模式仅支持导联场投影路径，不支持注意力模式")
    ...
    def forward(
        ...
        if self.use_fixed_leadfield:
            if self.projection_only:
                raise RuntimeError(
                    "projection_only 模式不支持 decoder.forward(source_activity, ...)。"
                    "请改用 project_source_state() 或 project_source_state_to_sensor()。"
                )
    ...
    def project_source_state(
        self,
        source_state: torch.Tensor,
        leadfield: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:
        """
        V1 显式状态投影接口（不经过 temporal_decoder）。
        ...
        参数:
            ...
            leadfield: 导联场矩阵
                - (N_sensors, N_sources)
                - (B, N_sensors, N_sources)
        返回:
            传感器信号
                - 输入 2D -> (N_sensors, T_state)
                - 输入 3D -> (B, N_sensors, T_state)
        """
        squeeze_batch = False
        if source_state.dim() == 2:
            source_state = source_state.unsqueeze(0)
            squeeze_batch = True
        if source_state.dim() != 3:
            raise ValueError(
                f"source_state 形状需为 (N,T) 或 (B,N,T)，收到: {tuple(source_state.shape)}"
            )

        L = self._resolve_leadfield(leadfield).to(source_state.device, dtype=source_state.dtype)
        if L.dim() == 2:
            sensor_state = torch.einsum("cs,bst->bct", L, source_state)
        elif L.dim() == 3:
            sensor_state = torch.einsum("bcs,bst->bct", L, source_state)
        else:
            raise ValueError(f"导联场形状不支持: {tuple(L.shape)}，预期 2D 或 3D")

        return sensor_state.squeeze(0) if squeeze_batch else sensor_state

    def project_source_state_to_sensor(
        self,
        source_state: torch.Tensor,
        leadfield: Optional[torch.Tensor] = None,
        ...
        参数:
            source_state: (N,T) 或 (B,N,T)
            leadfield: (C,N) 或 (B,C,N)
            target_length: 若给定且与 T 不一致，使用线性重采样对齐
        返回:
            (C,T_out) 或 (B,C,T_out)
        """
        sensor_signal = self.project_source_state(source_state, leadfield=leadfield)

        if target_length is None:
            return sensor_signal

        squeeze_batch = False
        if sensor_signal.dim() == 2:
            sensor_signal = sensor_signal.unsqueeze(0)
            squeeze_batch = True
        if sensor_signal.shape[-1] != target_length:
        ...
        return sensor_signal.squeeze(0) if squeeze_batch else sensor_signal
